...
if len(sys.argv) == 2 and len(sys.argv[1]) > 3:
    ...
    visit_b = randrange(1, n)
    exp_visitb = get_exp_vistb(visit_b)
    for tmp_student in student_list:
        i = 0
        while i < D:
            p = min(int(np.random.uniform(visit_b)+1), n)
    ...
    i = 0
    prom_t = np.random.randint(20)
    while i < n:
        Booth_promotion_time.append(int(min(1+np.random.exponential(prom_t), 20)))
        i = i + 1

    exp_pro_t = get_exp_pro(prom_t)
...
while current_day <= D:
    ...
    tot_tra_2day += total_traveling_time

exp_data = {"bo_num": [n], "exp_pro": [exp_pro_t], "exp_vis": [exp_visitb], "total_tra": [tot_tra_2day]}
exp_df = pd.DataFrame(data=exp_data)
exp_df.to_csv('dataf.csv', mode='a', header=False, index=False)
