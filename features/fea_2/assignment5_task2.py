...
class Booth:
    ...
    def set_next(self):
        ...
        if len(self.booth_queue) > 0:
            ...
            # check the len of people
            ql = len(self.booth_queue)
            ser_ls = []
            if ql <= 6:
                item = self.booth_queue
                self.booth_queue = []
                for i in item:
                    ser_ls.append(i[0])
                self.serving = ser_ls
            else: # >=7
                item = self.booth_queue[0:6]
                self.booth_queue = self.booth_queue[6:]
                for i in item:
                    ser_ls.append(i[0])
                self.serving = ser_ls

            # Extra for feature 2:
            find_com(self.serving)
            self.next_event_time = self.next_event_time + self.service_time
            self.idle = False
            #if len(self.booth_queue) == 0:
               # return 0
            #waiting_time = self.next_event_time - self.booth_queue[0][1]
            waiting_time = (self.next_event_time-self.service_time) - item[-1][1]
            return waiting_time
...
def find_com(st_ls):
    ...
    if rnd_int <= 3:
        pref_ls = []
        wh_ls = []
        # get the pref from each st to a ls
        for std in st_ls:
            pref_ls.append((std, student_list[int(std)].preference))
            wh_ls.extend(student_list[int(std)].preference)
        # find highest freq. common booth
        # check the freq > 1
        if len(wh_ls) == len(set(wh_ls)):
            return
        most_common_booth = max(wh_ls, key=wh_ls.count)

        for std_id, pref in pref_ls:
            if most_common_booth in pref:
                # swap to top
                most_ind = pref.index(most_common_booth)
                pref[0], pref[most_ind] = pref[most_ind], pref[0]
                student_list[std_id].set_preference(pref)
...
def call_event(student_list, booth_list, current_day, current_time, termination_time, total_booth, simulation_log):
    ...
    for tmp_booth in booth_list:
        if tmp_booth.idle and len(tmp_booth.booth_queue) > 0:
            ...
            for ser_st in tmp_booth.serving:
                if len(student_list[ser_st].preference) == 0:
                    student_list[ser_st].next_event_type = "departure"
                    sh_dis = 0
                    ...
                    student_list[ser_st].next_event_time = current_time + sh_dis
                    datum_bd = {"Day": current_day, "Time": change_to_time_format(current_time),
                             "Event": "Student " + str(
                                 int(student_list[ser_st].ID) + 1) + " departs at booth " + str(tmp_booth.ID+1), "len(Queue)": str(len(tmp_booth.booth_queue))}
                    simulation_log = simulation_log.append(datum_bd, ignore_index=True)
                    current_traveling_time += sh_dis
                else:

                    student_list[ser_st].next_event_type = "traveling"
                    student_list[ser_st].next_event_time = current_time + traveling_time(tmp_booth.ID,
                                                                                                    student_list[
                                                                                                        ser_st].preference[
                                                                                                        0])
                    datum_bt = {"Day": current_day, "Time": change_to_time_format(current_time),
                             "Event": "Student " + str(student_list[ser_st].ID + 1) + " departs at booth " + str(
                                 tmp_booth.ID + 1), "len(Queue)": str(len(tmp_booth.booth_queue))}
                    simulation_log = simulation_log.append(datum_bt, ignore_index=True)

                    current_traveling_time += traveling_time(tmp_booth.ID, student_list[ser_st].preference[0])
